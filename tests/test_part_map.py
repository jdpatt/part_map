...
class TestPartObject:
    def test_load_from_excel(self):
        obj = PartObject.from_excel(Path(__file__).parent.parent.joinpath("examples", "artix7_example.xlsx"))
        assert obj.get_number_of_pins() == 238
    
    def test_load_from_json(self):
        obj = PartObject.from_json(Path(__file__).parent.parent.joinpath("examples", "connector_example.json"))
